    ...
    <PremiumLayout user={{ name: 'Lenny De K.' }}>
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            ...
            <button 
              ...
              className="bg-white/10 rounded-xl p-4 backdrop-blur-sm text-left hover:bg-white/20 transition-colors cursor-pointer"
            >
              <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs text-pink-300">Tap voor ranks →</p>
            </button>
      ...
      {showRankModal && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          />
          <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-t-2xl sm:rounded-2xl shadow-2xl max-h-[90vh] overflow-hidden animate-slide-up">
            {/* Header */}
            <div className="sticky top-0 bg-gradient-to-r from-purple-600 to-blue-600 text-white p-4 sm:p-6 z-10">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold">Rank Overzicht</h2>
                    <p className="text-purple-200 text-sm">Alle ranks en vereisten</p>
                  </div>
                </div>
                <button 
                  onClick={() => setShowRankModal(false)}
            ...
            <div className="p-4 sm:p-6 overflow-y-auto max-h-[60vh] space-y-4">
              {MLM_RANKS.map((rank, index) => {
                const isCurrentRank = rank.rank === USER_DATA.currentRank;
                  ...
                  >
                    <div className="flex items-start gap-4">
                      <div className={`w-14 h-14 rounded-xl flex items-center justify-center flex-shrink-0 ${
                        rank.color === 'gray' ? 'bg-gray-100 text-gray-600' :
                        rank.color === 'blue' ? 'bg-blue-100 text-blue-600' :
                        rank.color === 'purple' ? 'bg-purple-100 text-purple-600' :
                        rank.color === 'orange' ? 'bg-orange-100 text-orange-600' :
                        rank.color === 'pink' ? 'bg-pink-100 text-pink-600' :
                        rank.color === 'indigo' ? 'bg-indigo-100 text-indigo-600' :
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-lg font-bold text-gray-900 dark:text-white">{rank.rank}</span>
                          {isCurrentRank && (
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">{rank.name}</p>
                        
                        <div className="grid grid-cols-3 gap-2 text-xs">
                          <div className="bg-gray-50 dark:bg-slate-800 p-2 rounded-lg text-center">
                            <p className="font-semibold text-gray-900 dark:text-white">{rank.directRequired}</p>
                            <p className="text-gray-500">Direct</p>
                          </div>
                          <div className="bg-gray-50 dark:bg-slate-800 p-2 rounded-lg text-center">
                            <p className="font-semibold text-gray-900 dark:text-white">{rank.teamRequired}</p>
                            <p className="text-gray-500">Team</p>
                          </div>
                          <div className="bg-gray-50 dark:bg-slate-800 p-2 rounded-lg text-center">
                            <p className="font-semibold text-gray-900 dark:text-white">{rank.aspRequired}</p>
                            <p className="text-gray-500">ASP</p>
                          </div>
                        </div>

                        {rank.infinityBonus.some(b => b > 0) && (
                          <div className="mt-2 p-2 bg-yellow-50 dark:bg-yellow-500/10 rounded-lg">
                            <p className="text-xs text-yellow-700 dark:text-yellow-400 font-medium">
                              Infinity Bonus: {rank.infinityBonus.filter(b => b > 0).join('€, ')}€
                            </p>
                          </div>
                        )}
                      </div>
            ...
            {/* Back Button */}
            <div className="sticky bottom-0 p-4 bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-700">
              <button 
                onClick={() => setShowRankModal(false)}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-gray-300 rounded-xl font-medium hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors"
              >
